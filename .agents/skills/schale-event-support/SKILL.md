...
description: Schale Inventory Managementでイベント開始に合わせて備品の種類・寸法・周回別数量・プリセット・多言語通知を更新するスキル。数量が未知の新規イベントと、過去データを再利用する復刻イベントの実装・検証・PR準備に使う。
---
...
- 初回、パターン切替点、最終の通常周回、「N周目以降」の各プリセットを適用すると、3品の高さ・幅・数量が根拠表と一致する。
- プリセット適用後に盤面と配置がリセットされ、確率計算を実行できる。
- 既存のlocalStorageがある場合でも、プリセットを明示的に適用して新データを確認する。保存済みの旧値を初期表示と誤認しない。
- 通知の文面、severity、issueリンクが正しい。
- 言語を切り替えて、選択肢の欠落や「以降」の誤訳がない。
